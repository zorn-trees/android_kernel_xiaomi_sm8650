...
#endif /* PROP_TXSTATUS || WL_DHD_XR_PCISDIO */
...
#endif // endif
#if defined(DHD_LB) || defined(WL_DHD_XR_PCISDIO)
...
#endif /* DHD_LB || WL_DHD_XR_PCISDIO */
...
#define wake_lock_init(lock, type, name)        wakeup_source_add(lock)
#define wake_lock_destroy(lock)                 wakeup_source_remove(lock)
#define wake_lock(lock)                         __pm_stay_awake(lock)
#define wake_lock_timeout(lock, timeout)        __pm_wakeup_event(lock, jiffies_to_msecs(timeout))
#define wake_unlock(lock)                       __pm_relax(lock)
#define wake_lock_active(lock)                  ((lock)->active)
#endif /* CONFIG_PM_WAKELOCKS || CONFIG_HAS_WAKELOCK */
