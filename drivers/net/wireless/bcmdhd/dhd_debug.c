...
dhd_dbg_set_event_log_tag(dhd_pub_t *dhdp, uint16 tag, uint8 set)
{
	...
	if (ret) {
		DHD_DBGIF(("%s set log tag iovar failed %d\n", __FUNCTION__, ret));
	}
}
	...
	dbg_mon_rx_pkts_t rx_pkt_mon)
{
	...
	DHD_DBG_RING_LOCK((void *)&dhdp->wl_start_stop_lock,flags);
	...
	if (PKT_MON_ATTACHED(tx_pkt_state) || PKT_MON_ATTACHED(tx_status_state) ||
		...
		DHD_DBG_RING_UNLOCK((void *)&dhdp->wl_start_stop_lock,flags);
	...
	dbg_urgent_noti_t os_urgent_notifier, void *os_priv)
{
	...
	ret = dhd_dbg_ring_init(dhdp, &dbg->dbg_rings[DEBUG_DUMP_RING_ID], DEBUG_DUMP_RING_ID,
			(uint8 *)DEBUG_DUMP_RING_NAME, DEBUG_DUMP_RING_SIZE, NULL, FALSE);
	if (ret)
		goto error;
