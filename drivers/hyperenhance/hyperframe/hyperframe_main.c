...
extern void (*hyperframe_notify_qos_cancel_fp)(void);
...
static void hyperframe_notifer_wq_cb(struct work_struct *psWork)
{
	...
	case HYPERFRAME_NOTIFIER_ID:
		...
		htrace_e_cpu_debug();
		break;
	case HYPERFRAME_NOTIFIER_QOS_CANCEL:
		htrace_b_sched_debug(current->tgid, "[HYPERSCHED#HYPERFRAME_NOTIFIER_QOS_CANCEL]");
		qos_reset_ret = hyperframe_reset_cpufreq_qos(0);
		htrace_b_sched_debug(current->tgid,
							"[HYPERFRAME#hyperframe_reset_cpufreq_qos] %d", qos_reset_ret);
...
void hyperframe_notify_qos_cancel(void) {
	if(!hyperframe_is_enable())
		return;

	if (!mPushes[HYPERFRAME_NOTIFIER_QOS_CANCEL])
		return;

	if (!hyperframe_notify_wq)
		return;

	mPushes[HYPERFRAME_NOTIFIER_QOS_CANCEL]->ePushType = HYPERFRAME_NOTIFIER_QOS_CANCEL;

	if (!work_pending(&mPushes[HYPERFRAME_NOTIFIER_QOS_CANCEL]->sWork))
		queue_work(hyperframe_notify_wq, &mPushes[HYPERFRAME_NOTIFIER_QOS_CANCEL]->sWork);
}
...
static int __init hyperframe_init(void)
{
	...
	hyperframe_notify_qos_cancel_fp = hyperframe_notify_qos_cancel;
