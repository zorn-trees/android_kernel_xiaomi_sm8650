...
#define CONFIG_HAVE_MULTI_SCREEN 1
#endif
	...
	struct panel_event_notification *notification, void *client_data)
{
	...
	if(notifier_tag == PANEL_EVENT_NOTIFICATION_PRIMARY){
		switch (notification->notif_type) {
			...
			pr_info("%s:DRM_PRIMARY_PANEL_EVENT_UNBLANK\n", __func__);
			if (gf_dev->device_available == 1) {
				gf_dev->fb_black = 0;
#if defined(GF_NETLINK_ENABLE)
			...
			pr_info("%s:DRM_PRIMARY_PANEL_EVENT_BLANK\n", __func__);
			if (gf_dev->device_available == 1) {
				gf_dev->fb_black = 1;
...
#if IS_ENABLED(CONFIG_HAVE_MULTI_SCREEN)
		if(notifier_tag == PANEL_EVENT_NOTIFICATION_SECONDARY){
		switch (notification->notif_type) {
			case DRM_PANEL_EVENT_UNBLANK:
			pr_info("%s:DRM_SECOND_PANEL_EVENT_UNBLANK\n", __func__);
			if (gf_dev->device_available == 1) {
				gf_dev->fb_black = 0;
#if defined(GF_NETLINK_ENABLE)
				temp[0] = GF_NET_EVENT_FB_UNBLACK;
			...
			pr_info("%s:exit\n", __func__);
				break;
			case DRM_PANEL_EVENT_BLANK:
			pr_info("%s:DRM_SECOND_PANEL_EVENT_BLANK\n", __func__);
			if (gf_dev->device_available == 1) {
				gf_dev->fb_black = 1;
				gf_dev->wait_finger_down = true;
#if defined(GF_NETLINK_ENABLE)
