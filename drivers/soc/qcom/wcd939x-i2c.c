...
static void wcd_usbss_equ_bw_settings_param(struct wcd_usbss_ctxt *priv)
{
	...
	if(((platform_id == HARDWARE_PROJECT_N2) && (build_major == 1) && (build_minor >= 1)) || ((platform_id == HARDWARE_PROJECT_N2) && (build_major == 2)))
		rc = of_property_read_u32(priv->dev->of_node, "wcd-equ-bw-settings-new-host", &priv->wcd_equ_bw_settings_host);
	else if (platform_id == HARDWARE_PROJECT_N2 && build_major == 9)
		rc = of_property_read_u32(priv->dev->of_node, "wcd-equ-bw-settings-mp-host", &priv->wcd_equ_bw_settings_host);
	else
		rc = of_property_read_u32(priv->dev->of_node, "wcd-equ-bw-settings-host", &priv->wcd_equ_bw_settings_host);

	if (rc)
		dev_err(priv->dev, "error reading the value of host bw settings\n");

	dev_err(priv->dev, "wcd_equ_bw_settings device is %X, host is %X\n", priv->wcd_equ_bw_settings, priv->wcd_equ_bw_settings_host);
}
